    ...
    try {
      ...
      switch(functionName) {
        ...
        case 'getMonthlyDetail':
          ...
          break;
        case 'getMonthlyPaymentDetail':
          result = getMonthlyPaymentDetail(params.month);
          break;
        case 'getMoveInOutByMonth':
          result = getMoveInOutByMonth(params);
          break;
        case 'getBadDebtors':
 ...
 */
  ...
  function runAll_Part1(){
    ...
    try {
      ...
      // ★ 반드시 JSON 반환!
      return { success: true, message: `runAll_Part1 완료: ${processedCount}개 호실, ${duration}초` };
...
function handleTelegramUpdate(update) {
  try {
    ...
    if (/^(\d{1,2})월이사$/.test(text)) {
      Logger.log('n월이사 분기 진입: ' + text);
      var monthMatch = text.match(/^(\d{1,2})월이사$/);
      var targetMonth = parseInt(monthMatch[1], 10);
      var targetYear = new Date().getFullYear();
      var ss = SpreadsheetApp.getActiveSpreadsheet();
      // === [수정] DATA_SHEET 상수 선언 또는 직접 시트명 입력 ===
      var sh = ss.getSheetByName(typeof DATA_SHEET !== 'undefined' ? DATA_SHEET : '입주데이터');
      var infoData = sh.getRange(2, 1, sh.getLastRow() - 1, 14).getValues();
      var 입주호실 = [];
      var 퇴실호실 = [];
      infoData.forEach(function(row) {
        ...
        var moveIn = row[4];
        var moveOut = row[5];
        // === [수정] parseKoreanDate 함수가 반드시 존재해야 함 ===
        var moveInDate = moveIn ? parseKoreanDate(moveIn) : null;
        var moveOutDate = moveOut ? parseKoreanDate(moveOut) : null;
        if (moveInDate && moveInDate.getFullYear() === targetYear && (moveInDate.getMonth() + 1) === targetMonth) {
          입주호실.push([room, name, contact, Utilities.formatDate(moveInDate, 'Asia/Seoul', 'yyyy-MM-dd'), moveOutDate ? Utilities.formatDate(moveOutDate, 'Asia/Seoul', 'yyyy-MM-dd') : '-']);
        }
        if (moveOutDate && moveOutDate.getFullYear() === targetYear && (moveOutDate.getMonth() + 1) === targetMonth) {
          퇴실호실.push([room, name, contact, moveInDate ? Utilities.formatDate(moveInDate, 'Asia/Seoul', 'yyyy-MM-dd') : '-', Utilities.formatDate(moveOutDate, 'Asia/Seoul', 'yyyy-MM-dd')]);
        }
      });
      Logger.log('입주호실: ' + JSON.stringify(입주호실));
      Logger.log('퇴실호실: ' + JSON.stringify(퇴실호실));
      var msg = '입주호실\n호실 | 성함 | 연락처 | 입주일 | 퇴실일\n';
      if (입주호실.length === 0) {
        msg += '없음\n';
      } else {
        입주호실.forEach(function(r) { msg += r.join(' | ') + '\n'; });
      }
      msg += '\n퇴실호실\n호실 | 성함 | 연락처 | 입주일 | 퇴실일\n';
      if (퇴실호실.length === 0) {
        msg += '없음';
      } else {
        퇴실호실.forEach(function(r) { msg += r.join(' | ') + '\n'; });
      }
      sendTelegramTo(chatId, msg.trim());
      return;
    }
    ...
    if (text === '공실') {
      sendTelegramTo(chatId, '공실 기능 결과 예시');
      return;
    }
    if (text === '미납') {
      sendTelegramTo(chatId, '미납 기능 결과 예시');
      return;
    }
    // ... (기타 기존 명령어 분기들) ...
...
function getAllRoomStatus(params = {}) {
  ...
  // 1. 기본정보 매핑 (2~124행, A~N열)
  const infoData = sh.getRange(2, 1, 123, 14).getValues();
  const infoMap = {};
  ...
  // 블록 구조에 맞는 인덱스
  const IDX = {
    TOTAL_ARREARS: 13, // 총미납금 (14번째 줄, 0부터 시작)
    REMAIN: 14        // 남은금액 (15번째 줄, 0부터 시작)
  };

  // 시트의 실제 호실 순서(블록 순서) 추출
  const colA = sh.getRange(DATA_START_ROW, 1, DATA_END_ROW - DATA_START_ROW + 1, 1).getValues().flat().map(String);

  rooms.forEach((room) => {
    const info = infoMap[room];
    if (!info || !info.name || !info.contact) return;
    const name = (info.name || '').toLowerCase();
    if (name.includes('시행사') || name.includes('공실') || name.includes('숙소')) return;
    // 입주 한달 미만 제외
    if (info.moveIn) {
      let moveInDate = info.moveIn;
      if (!(moveInDate instanceof Date)) {
        moveInDate = parseKoreanDate(moveInDate);
      }
      if (moveInDate && moveInDate > oneMonthAgo) return;
    }
    // 실제 시트 블록 순서로 계산
    const blkIdx = colA.filter(r => r).indexOf(room);
    if (blkIdx < 0) return;
    const start = REPORT_BASE + blkIdx * 17; // 17행 블록
    // 로그로 실제 읽는 셀 위치와 값 확인
    Logger.log(`[${room}] start=${start}, TOTAL_ARREARS=${start+IDX.TOTAL_ARREARS}, REMAIN=${start+IDX.REMAIN}`);
    Logger.log(`[${room}] TOTAL_ARREARS cell: ` + sh.getRange(start + IDX.TOTAL_ARREARS, 3).getValue());
    Logger.log(`[${room}] REMAIN cell: ` + sh.getRange(start + IDX.REMAIN, 3).getValue());
    const totalUnpaid = parseFloat(sh.getRange(start + IDX.TOTAL_ARREARS, 3).getValue()) || 0; // 14번째 행, C열
    const totalSettle = parseFloat(sh.getRange(start + IDX.REMAIN, 3).getValue()) || 0; // 15번째 행, C열
    // 정산금액이 maxWon보다 크면 제외 (마이너스 포함)
    if (params && params.maxWon !== undefined && totalSettle >= params.maxWon) return;
    res.push({
      room,
      name: info.name,
      contact: info.contact,
      remain: totalSettle, // 총정산금액
      unpaid: totalUnpaid, // 총미납금
      note: info.note,
      moveIn: info.moveIn,
    });
  });
...
function getMonthlyDetail(month) {
  try {
    ...
    // 입금데이터 시트에서 월별 입금일 매핑 (room|month → [날짜들])
    let payMap = {};
    if (paySh && paySh.getLastRow() > 1) {
      const payData = paySh.getRange(2, 1, paySh.getLastRow() - 1, 6).getValues();
      payData.forEach(row => {
        const room = String(row[0]).trim();
        const date = row[4];
        if (!room || !date) return;
    ...
    rooms.forEach((room, idx) => {
      ...
      const headerRow = sh.getRange(start, 3, 1, sh.getLastColumn() - 2)
        .getDisplayValues()[0]
      ...
      // 입금일 추출
      let paymentDate = '';
      const key = `${room}|${month}`;
      if (payMap[key] && payMap[key].length > 0) {
        // 가장 최근 입금일
        const latest = payMap[key].sort((a, b) => b - a)[0];
        paymentDate = Utilities.formatDate(latest, tz, 'yyyy-MM-dd');
      }
      if (!room) return;
      res.push({ room, name, contact, billing, payment, settle, remark, paymentDate });
...
function getMonthlyPaymentDetail(month) {
  try {
    Logger.log('==== getMonthlyPaymentDetail 진입 ====');
    Logger.log('입력 month: ' + month);
    ...
    Logger.log('PAYMENT_SHEET 변수: ' + (typeof PAYMENT_SHEET !== 'undefined' ? PAYMENT_SHEET : 'undefined'));
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const paySh = ss.getSheetByName(typeof PAYMENT_SHEET !== 'undefined' ? PAYMENT_SHEET : '입금데이터');
    Logger.log('paySh 존재 여부: ' + !!paySh);

    const tz = Session.getScriptTimeZone();
    if (!paySh) {
      Logger.log('입금데이터 시트를 찾을 수 없습니다');
      return { success: false, message: '입금데이터 시트를 찾을 수 없습니다' };
    }

    Logger.log('paySh.getLastRow(): ' + paySh.getLastRow());
    if (paySh.getLastRow() < 2) {
      Logger.log('입금데이터 시트에 데이터가 없습니다.');
      return { success: false, message: '입금데이터 시트에 데이터가 없습니다.' };
    }

    const payData = paySh.getRange(2, 1, paySh.getLastRow() - 1, 6).getValues();
    Logger.log('payData.length: ' + payData.length);
    ...
    payData.forEach((row, idx) => {
      ...
      let amount = row[1];
      const dateRaw = row[4];
      if (!room || !amount || !dateRaw) return;

      // 날짜 파싱 (YYYY-MM-DD or YYYY. M. D. → YYYY-MM-DD)
      let dateObj = new Date(dateRaw);
      if (isNaN(dateObj.getTime())) {
        // 한글형식 등 처리
        const m = String(dateRaw).match(/(\\d{4})[.\\-\\/\\s]+(\\d{1,2})[.\\-\\/\\s]+(\\d{1,2})/);
        if (m) dateObj = new Date(`${m[1]}-${m[2].padStart(2,'0')}-${m[3].padStart(2,'0')}`);
      }
      if (isNaN(dateObj.getTime())) return; // 날짜 파싱 실패시 skip
    ...
    Logger.log('고유 호실 수: ' + roomSet.size);
    Logger.log('전체 입금합계: ' + totalPayment);
    Logger.log('payments.length: ' + payments.length);
...
function getMoveInOutByMonth(params) {
  try {
    var year = params && params.year ? params.year : new Date().getFullYear();
    var month = params && params.month;
    if (!month) return { success: false, message: '월 정보가 없습니다.' };

    var sheet = SpreadsheetApp.getActive().getSheetByName('입주데이터');
    var data = sheet.getDataRange().getValues();

    // 헤더 인덱스 찾기
    var headers = data[0];
    var idxRoom = headers.indexOf('호수') !== -1 ? headers.indexOf('호수') : headers.indexOf('호실');
    var idxName = headers.indexOf('성함');
    var idxContact = headers.indexOf('연락처');
    var idxMoveIn = headers.indexOf('입주일');
    var idxMoveOut = headers.indexOf('퇴실일');
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      var contact = row[idxContact];
      var moveIn = row[idxMoveIn];
      var moveOut = row[idxMoveOut];

      // 데이터 유효성 검사
      if (!room || room === '' || room === undefined) continue;
      if (!name || name === '' || name === undefined) continue;
      if (!contact || contact === '' || contact === undefined) continue;
      var nameLower = String(name).toLowerCase();
      if (nameLower.includes('시행사') || nameLower.includes('숙소') || nameLower.includes('공실')) continue;

      // 날짜 파싱 (문자열이면 Date로 변환)
      var moveInDate = null, moveOutDate = null;
      if (moveIn) {
        if (typeof moveIn === 'object' && moveIn.getFullYear) {
          moveInDate = moveIn;
        } else if (typeof moveIn === 'string' && moveIn.match(/^\d{4}-\d{2}-\d{2}$/)) {
          moveInDate = new Date(moveIn + 'T00:00:00+09:00');
        }
      }
      if (moveOut) {
        if (typeof moveOut === 'object' && moveOut.getFullYear) {
          moveOutDate = moveOut;
        } else if (typeof moveOut === 'string' && moveOut.match(/^\d{4}-\d{2}-\d{2}$/)) {
          moveOutDate = new Date(moveOut + 'T00:00:00+09:00');
        }
      }

      // 입주일 체크
      if (moveInDate && moveInDate.getFullYear() == year && (moveInDate.getMonth() + 1) == month) {
        moveInList.push({
          room: room,
          name: name,
          contact: contact,
          moveIn: Utilities.formatDate(moveInDate, 'Asia/Seoul', 'yyyy-MM-dd'),
          moveOut: moveOutDate ? Utilities.formatDate(moveOutDate, 'Asia/Seoul', 'yyyy-MM-dd') : ''
        });
      }
      // 퇴실일 체크
      if (moveOutDate && moveOutDate.getFullYear() == year && (moveOutDate.getMonth() + 1) == month) {
        moveOutList.push({
          ...
          name: name,
          contact: contact,
          moveIn: moveInDate ? Utilities.formatDate(moveInDate, 'Asia/Seoul', 'yyyy-MM-dd') : '',
          moveOut: Utilities.formatDate(moveOutDate, 'Asia/Seoul', 'yyyy-MM-dd')
        });
      }
    }
    ...
    return { success: false, message: 'getMoveInOutByMonth 오류: ' + e };
  }
}
